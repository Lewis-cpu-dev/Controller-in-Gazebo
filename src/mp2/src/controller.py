...
class vehicleController():
    ...
    def longititudal_controller(self, curr_x, curr_y, curr_vel, curr_yaw, future_unreached_waypoints):
        ...
        bounds = [0.4,0.7]
        if num_points>10:
            recent_points=future_unreached_waypoints[0:10]
        else:
            recent_points=future_unreached_waypoints[0:num_points]
        speeds =[]
        for i,wp in enumerate(recent_points):
            wp_x, wp_y = wp
        
            # Compute the desired heading from the current position to the waypoint
            desired_yaw = math.atan2( wp_y-curr_y, wp_x-curr_x)
            # print("desired:",desired_yaw)
            # print("current:",curr_yaw)
            # Normalize the difference to the range [0, pi]
            angle_diff = abs((desired_yaw - curr_yaw + math.pi) % (2 * math.pi) - math.pi)
            # angle_diff = abs(desired_yaw-curr_yaw)
            target_speeds = [13,18,25]
            # Mapping: 
            ...
            if angle_diff < bounds[0]:    #
                ...
                if curr_vel >=(target_speeds[1]-1):
                    target_speed = target_speeds[2]
            elif angle_diff > bounds[1]:    
                target_speed = target_speeds[0]
            elif angle_diff < bounds[1] and angle_diff > bounds[0]:
                target_speed = (target_speeds[1]-target_speeds[0])*(1-(angle_diff-bounds[0])/((bounds[1] - bounds[0])))\
                +target_speeds[0]
            
            speeds.append(target_speed)

            # if angle_diff > bounds[1]:   
            #     target_speed = target_speeds[1]
            #     if curr_vel >=target_speeds[2]:
            #         target_speed = target_speeds[2]
            # elif angle_diff < bounds[0]:    
            #     target_speed = target_speeds[0]
            # elif angle_diff < bounds[1] and angle_diff > bounds[0]:
            #     target_speed = (target_speeds[1]-target_speeds[0])*(1- (angle_diff-bounds[0])/((bounds[1] - bounds[0])))\
            #     +target_speeds[0]
    ...
    # Choose the minimum speed from all evaluated waypoints to ensure safety.
        ...
        # print(angle_diff)
        return target_velocity, angle_diff
    ...
    def execute(self, currentPose, target_point, future_unreached_waypoints):
        ...
        target_velocity,angle_diff = self.longititudal_controller(curr_x, curr_y, curr_vel, curr_yaw, future_unreached_waypoints)
