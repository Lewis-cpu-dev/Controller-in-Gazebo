...
def run_model():
    ...
    while not rospy.is_shutdown():
        ...
        if (distToTargetX < 2 and distToTargetY < 2):
            ...
            print(f"Time Taken: {round(time_taken, 2)}", "reached",\
            pos_list[prev_pos_idx][0],pos_list[prev_pos_idx][1],"next",pos_list[pos_idx][0],pos_list[pos_idx][1],\
            "velocity:", current_vel,"angle diff:",angle_diff)

        current_vel,angle_diff = controller.execute(currState, [target_x, target_y], pos_list[pos_idx:])
